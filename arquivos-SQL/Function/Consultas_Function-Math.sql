...
SELECT NUMERO, QUANTIDADE, PRECO
 ...
 FROM itens_notas_fiscais;
  ...
  SELECT YEAR(DATA_VENDA),FLOOR(SUM((QUANTIDADE * PRECO)* IMPOSTO)) AS TRIBUTAÇÃO
  FROM notas_fiscais NF  INNER JOIN itens_notas_fiscais INF ON 
  NF.NUMERO = INF.NUMERO WHERE YEAR(DATA_VENDA) = 2016
